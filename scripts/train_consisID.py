...
def compute_log_prob(transformer,pipeline, sample, attention_kwargs, j, config,rank):
    ...
    # if j == 10:
    #     print("====pipeline_with_prob====t:",sample["timesteps"][:, j])
    #     print(f"[{rank}]latent_model_input:",latent_model_input[0][0][0])
    #     print(f"[{rank}]prompt_embeds:",torch.cat([sample["negative_prompt_embeds"], sample["prompt_embeds"]], dim=0)[0][0])
    #     print(f"[{rank}]timestep:",sample["timesteps"][:, j][0].expand(latent_model_input.shape[0]))
    #     print(f"[{rank}]image_rotary_emb:",sample["image_rotary_emb"])
    #     print(f"[{rank}]id_vit_hidden:",sample["id_vit_hidden"][0][0][0])
    #     print(f"[{rank}]id_cond:",sample["id_cond"][0])
    ...
    #     print(f"[{rank}][before ddim] latents:",sample["latents"][:, j][0][0][0])
    #     print(f"[{rank}][before ddim] noise_level:",config.sample.noise_level)
    ...
    # if j == 10:
    #     print(f"[{rank}][after ddim] noise_pred:",prev_sample[0][0])
...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters,
    ...
    for i,test_batch in tqdm(
        ...
        if config.sample.same_latent:
            generator = create_generator(prompts, base_seed=i)
        else:
            generator = None
...
def preprocess_batch(image_urls, prompts, negative_prompt, do_classifier_free_guidance, max_sequence_length, device,config,
    ...
    embeds, neg_embeds = pipeline.encode_prompt(
        prompt=prompts,
        negative_prompt=negative_prompt,
        do_classifier_free_guidance=do_classifier_free_guidance,
    ...
    # 2. image latents
    cropped_imgs = pipeline.video_processor.preprocess(align_crop_face_imgs, height=height, width=width).to(
        device, dtype=embeds.dtype
    )
    latents, image_latents = pipeline.prepare_latents(
        cropped_imgs, # torch.Size([1, 3, 480, 720])
        embeds.shape[0] * 1, # 1
        pipeline.transformer.config.in_channels // 2, # 16
        height=height, # 480
        width=width, # 720
        dtype=embeds.dtype, # torch.bfloat16
    ...
    if pipeline.transformer.config.use_rotary_positional_embeddings:
        ...
        image_rotary_emb = image_rotary_emb.expand(latents.size(0), -1, -1, -1)
    else:
    ...
    return align_crop_face_imgs, valid_id_vit_hiddens, valid_id_conds, embeds, neg_embeds, image_latents,image_rotary_emb
...
def main(_):
    ...
    pipeline.transformer.to(accelerator.device, dtype=inference_dtype) # TODO pipeline.transformer.to(accelerator.device)
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        with autocast():
                            prev_sample, log_prob, prev_sample_mean, std_dev_t = compute_log_prob(transformer,pipeline, sample,attention_kwargs=None,j=j,config=config,rank=accelerator.process_index) # pipeline, sample, attention_kwargs, j, config):
                            if config.train.beta > 0:
                                with torch.no_grad():
                                    with transformer.module.disable_adapter():
                                        _, _, prev_sample_mean_ref, _ = compute_log_prob(transformer,pipeline, sample,attention_kwargs=None,j=j,config=config,rank=accelerator.process_index)
                        ...
                        print("log_prob:",log_prob,"sample['log_probs'][:, j]:",sample["log_probs"][:, j])
                        print("ratio:",ratio-1)
